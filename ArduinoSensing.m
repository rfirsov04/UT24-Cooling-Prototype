...
my_structure.voltageA0 = zeros(4000, 1);
my_structure.voltageA1 = zeros(4000, 1);
my_structure.voltageA2 = zeros(4000, 1);
my_structure.voltageA3 = zeros(4000, 1);
my_structure.voltageA4 = zeros(4000, 1);
my_structure.voltageA5 = zeros(4000, 1);
...
axis([0 inf 1.4 2.1]) %Chose those y-axis values because in normal conditions, temp is aprox 22ºC
i = i + 1;
while(flag ~= '\') %For making the loop infinite
  tic
...
save tempzener15A4Soutlet0.mat -struct my_structure %or just save it every iteration of the while loop so we don't need flag which delays by like 10 seconds